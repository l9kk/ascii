        ...
        if arg.startswith('--'):
            if arg.startswith('--align='):
                ...
                if color not in ANSI_COLOR_CODES:
                    print(f"Invalid color '{color}'. Available colors: {', '.join(ANSI_COLOR_CODES.keys())}")
                    return
                index +=1
                ...
                print("Usage: python main.py [options] [string] [font] || Example: python main.py --color=red \"test\" standard || Options: --output=, --align=, --color=")
                return
        else:
            break
        ...
        if output_file:
            try:
                with open(output_file, 'w', encoding='utf-8') as created_file:
                    print_ascii_art_align(sep_args, lines, align, width, created_file, color, letters_to_color)
            except IOError:
                print("Something went wrong while creating output file.")
        else:
            print_ascii_art_align(sep_args, lines, align, width, None, color, letters_to_color)
    else:
        if output_file:
            try:
                with open(output_file, 'w', encoding='utf-8') as created_file:
                    print_ascii_art_to_file(sep_args, lines, created_file, color, letters_to_color)
            except IOError:
                print("Something went wrong while creating output file.")
        else:
...
def print_ascii_art_align(sentences, text_file, position, w, output_file, color, letters_to_color):
    for word in sentences:
        ...
        for h in range(1, 9):
            ...
            for char in word:
                if char == ' ':
                    line_output += ' ' * 8  # Adjust space width as needed
                else:
                    ...
                    if 0 <= line_index < len(text_file):
                        ...
                        if color and (not letters_to_color or char in letters_to_color):
                            line = ANSI_COLOR_CODES[color] + line + ANSI_COLOR_CODES['reset']
                        line_output += line
